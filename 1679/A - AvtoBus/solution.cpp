...
void solve()
{
    rll(n);
 
    //4x+6y=n ..find max and min value of (x+y)...x>=0,y>=0
 
    if(n&1 || n<4){
        pln(-1);
        return;
...
 
    // for max val use all 4's
    // remainder 0 2  to think about (odd remainders handeled in odd n)
    // remainder 0 use all 4s
    // remainder 2 , ex  6 mod 4  = 2 ..  replace one 4 with one 6..so (x+y)=n/4 remain constant
    
    ll maxval= n/4; 
 
    //for min val use all 6's
    //remainders 0,2,4
    // for remainder 0, use all 6's
    // for remainder 2,   say 8%6=2...  replace one 6 with two 4's .. x+y= (n/6)+1;   
    // for remainder 4,   say 10%6=2...  don't replace  but add one 4 .. x+y= (n/6)+1; 
    ll minval=0;
    if(n%6==0) minval=n/6;
 
    if(n%6==2) minval=(n/6)+1;
 
    if(n%6==4) minval= (n/6)+1;
 
    p(minval);sp;p(maxval);ln;
 
 
 
 
