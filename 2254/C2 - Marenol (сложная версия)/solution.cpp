...
 
   // f(i,1,n-1){
   //    if(a[i-1]==a[i+1]  && a[i-1]!=b[i-1] && a[i-1]!=b[i+1]){
   //       pln("NO");
 
   //       return;
   //    }
...
 
   ...
   f(i,0,n){
      ...
      if(a[i]=='1'){
         if(i&1) {cntoa++; oa.pb(i);}
         else {cntea++; ea.pb(i);}
      }
      if(b[i]=='1'){
         if(i&1) {cntob++; ob.pb(i);}
         else {cnteb++; eb.pb(i);}
      }
   }
...
 
 ...
 //   int jumpe=0;
 //   int jumpo=0;
 //   if(inde.size()!=0)  jumpe= inde[inde.size()-1]-inde[0];
 //    if(indo.size()!=0) jumpo= indo[indo.size()-1]-indo[0];
 
 //   jumpe=jumpe/2;
 //   jumpo=jumpo/2;   
 //   pln(jumpe+jumpo);
