...
 
    ...
    //     ll right=suf[h];
 
    //     debug(left,right,l,h);
    //     ll reduce=max(left,right);
    //             debug(reduce);
    //     if(reduce==left){
    //         ll we=ceil(reduce/(x * 1.0));
    //          debug(we);
    //         debug(suf2[l+1]);
    //         ll wq=suf2[l+1]; 
    //         debug(wq);
...
 
    ...
    //         minb=min(minb,  we+wq);
    //                 debug(minb);
    //        // minb=max(minb,  ceil(reduce/(x * 1.0)) + ceil(pre[h-1]/(x *       debug(wq+we);1.0)));
    //     }
