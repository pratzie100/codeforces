...
void solve()
{
	rll(n);
 
	//divisibility by 6 has to be caused by factors 2 and 3
	//means current n must be divisible by 3
	//1) now if current n is not divisible by 6,
	// then we only have choice of multiply by 2
	// 1a) if current n is itself even, and multiplied by 2 (again even)
	// then a number divisible by 6 can never be formed by such combination
	//one of them has to be odd //6 =3 * 2 
	// 1b) if current n is odd, then we have choice of multiply by 2
	// just like n=3..we can make ut multiply by 2 to make it 6
	// then later on divide by 6
 
	//2) current n is divisible by 6 simply reduce by dividing by 6 and increase count
 
	ll c=0;
	while(n>1){
		if(n%6!=0 && !(n&1)){
