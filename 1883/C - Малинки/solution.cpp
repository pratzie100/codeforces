...
 
        ans=min(ans,   k -  (x%k) ); //for 2,3,5 and 4
        
        if(x%2==0) ce++;  //for 4, additional case (should have atleast 2 even numbers)
    }
    
    int neededfor4 =  (ce>2) ?  0 : max(0, 2- ce);  // 2n * 2n = 4n
    
    if(k==4) {pln( min(ans,neededfor4)); return;}
    pln(ans);
}  
 
 
